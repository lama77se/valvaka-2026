...
export function ResultPanel() {
  ...
  const selectValue =
    selectedArea.level === 'riket' ? 'riket' : `${selectedArea.level === 'region' ? 'r' : 'k'}:${selectedArea.code}`

  return (
    <div className="flex h-full flex-col gap-3 overflow-hidden">
      {/* Områdesväljare: Riket + region/kommun-drilldown (valtyp styrs från kartan) */}
      <select
        ...
        onChange={(e) => {
          ...
          const next: Area =
            v === 'riket' ? RIKET : { level: v.startsWith('r:') ? 'region' : 'kommun', code: v.slice(2) }
