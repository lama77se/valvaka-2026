// Fas 5 — simulerad valnatt: upsertar RD-resultat i klungor så kartklienten kan
// visa realtidsfärgning. INTE riktig data — bara för att bevisa Realtime→kart-paint
// tills 2026-filerna finns. Fas 6 gör riktig 2022-replay via result_snapshot.
//
//   node --env-file=.env.local scripts/simulate-valnatt.mjs [--districts N] [--batch N] [--delay ms]
//
...
log(`${parties.length} färgade partier, ${districts.length} distrikt, batch ${BATCH}, delay ${DELAY} ms`)

let done = 0
for (let i = 0; i < districts.length; i += BATCH) {
  const rows = []
  for (const d of districts.slice(i, i + BATCH)) {
    // Slumpa ett distrikts profil: skala varje partis bas med brus.
  ...
  log(`${done}/${districts.length} distrikt inrapporterade`)
